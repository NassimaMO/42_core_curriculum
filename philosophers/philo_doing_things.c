...
int	dying(t_philosophers **philosopher)
{
	...
	if ((*philosopher)->data->philo_stop)
		return (1);
	if (((curr_time - (*philosopher)->data->time) + ((*philosopher)->last_eaten - (*philosopher)->data->time)) >= (*philosopher)->data->time_to_die)
		return (print_lock(philosopher, "died", curr_time), pthread_mutex_lock(&(*philosopher)->data->print), usleep((*philosopher)->data->time_to_die), \
		pthread_mutex_unlock(&(*philosopher)->data->print), (*philosopher)->data->philo_stop++, 1);
	return (0);
}
...
void	eating(t_philosophers **philosopher)
{
	...
	print_lock(philosopher, "has taken a fork", current_time());
	printf("%ld %d is eating\n", current_time() - (*philosopher)->data->time, (*philosopher)->philo_nbr);
