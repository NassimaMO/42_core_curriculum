...
static void	lock_fork(t_philo *philo, t_data *data)
{
	while (1)
	{
		...
		(philo->philo_nbr == data->nbr_philos) * (data->nbr_philos - 1)]);
		pthread_mutex_lock(&data->forks[(philo->philo_nbr - 1) * \
		(philo->philo_nbr < data->nbr_philos)]);
		if (!data->tab_forks[philo->philo_nbr - 1] && \
		!data->tab_forks[philo->philo_nbr % data->nbr_philos])
			break ;
...
{
	if (data->nbr_philos == 1 && (print_lock(philo, data, FORK), usleep(1000 * \
	(data->time_to_die - (current_time() - data->time))), 1))
